...
function subscribe_button($plan)
{
    ...
    if (auth()->user()) {
        ...
        if ($user->isSubscribed() && $plan->id == $subscription->plan->id && $subscription->isFree()) {
            $button = '<button class="button ' . $buttonClass . ' text-white -lg w-100 mt-16 transform-none disabled">' . lang('Active', 'plans') . '</button>';
 ...
 * @return string
 */
function file_upload($file, $location, $specificName = null, $old = null)
{
    make_directory($location);
    if (!empty($location.$old)) {
        remove_file($location.$old);
...
function quick_switch($title, $id, $checked = false, $hint = ''){
    ...
    echo '<div class="form-group">
        ...
        <div class="form-toggle-option">
            <div>
                <label for="'.$id.'">'.admin_lang("Enable").'</label>
            </div>
            <div>
