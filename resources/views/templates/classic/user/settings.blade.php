...
@section('content')
    <div class="d-flex justify-content-between align-items-center pb-30">
        <div class="title-head">
            <h1 class="mb-0">{{ lang('Settings') }}</h1>
        </div>
    </div>
    <div class="card">
                ...
                <div>
                    <form id="deatilsForm" action="{{ route('editProfile') }}#edit_profile" method="POST"
                        ...
                        <div class="row g-3 mb-24">
                            <div class="col">
                                <label for="firstname" class="form-label fw-bold">{{ lang('First Name') }} *</label>
                                <input type="text" class="form-control text-field" name="firstname" id="firstname" maxlength="50" placeholder="{{ lang('First Name') }}"
                                       value="{{ $user->firstname }}" required>
                            </div>
                            <div class="col">
                                <label for="lastname" class="form-label fw-bold">{{ lang('Last Name') }} *</label>
                                <input type="text" name="lastname" class="form-control text-field"
                                       placeholder="{{ lang('Last Name') }}" maxlength="50"
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-6">
                                <div class="mb-3">
                                    <label for="email" class="form-label fw-bold">{{ lang('Email address') }} *</label>
                                    <input type="email" name="email" class="form-control text-field"
                                           placeholder="{{ lang('Email address') }}" value="{{ $user->email }}"
                                           required>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-4">
                                <div class="mb-3">
                                    <label class="form-label fw-bold">{{ lang('Postal Code') }} *</label>
                        ...
                        <button type="submit" class="mt-24 button -lg -primary rounded-pill w-100">{{ lang('Save Changes') }}</button>
                    </form>
                </div>
            </div>
                ...
                <div class="pt-30">
                    <div class="settings-box-body">
                        ...
                        <p class="text-muted mb-2">
                            {{ lang('Open the authentication app (ex: Authy, Google Authenticator) on your mobile device and scan the following QR Code with your camera.') }}
                        </p>
                        @if (!$user->google2fa_status)
                            <div class="d-flex flex-column flex-md-row align-items-center">
                                ...
                                <div>
                                    <label>{{ lang("Can't scan the QR Code?") }}</label>
                                    <p class="text-muted">{{ lang("Try inserting the following secret code into your app if you can't scan the QR Code.") }}</p>

                                    <p class="h5">{{ $user->google2fa_secret }}</p>
                                </div>
                            </div>
                            <a href="#" class="mt-24 button -lg -primary rounded-pill w-100" data-bs-toggle="modal"
                               data-bs-target="#towfactorModal">{{ lang('Enable 2FA Authentication') }}</a>
                        @else
                            <a href="#" class="mt-24 button -lg -danger rounded-pill w-100" data-bs-toggle="modal"
                               data-bs-target="#towfactorDisableModal">{{ lang('Disable 2FA Authentication') }}</a>
                        @endif
                    </div>
                </div>
            </div>
                ...
                <div class="modal-content">
                    <form action="{{ route('2fa.enable') }}#auth" method="POST">
                        ...
                        <div class="modal-body">
                            <div class="mb-3">
                                <label class="form-label">{{ lang('OTP Code') }} : <span
                            ...
                            <div class="d-flex justify-content-between">
                                <button type="submit"
                                        class="button -primary w-100 me-2">{{ lang('Enable') }}</button>
                ...
                <div class="modal-content">
                    <form action="{{ route('2fa.disable') }}#auth" method="POST">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="d-flex justify-content-between">
                                <button type="submit"
                                        class="button -primary w-100 me-2">{{ lang('Disable') }}</button>
                                <button type="button" class="button -secondary w-100 ms-2"
                                        data-bs-dismiss="modal">{{ lang('Close') }}</button>
                            </div>
                        </div>
