    ...
    <div class="blog-sidebar ms-lg-auto">
        <form action="{{ route('blog.index') }}" method="GET">
            <div class="search-input position-relative mb-30">
                ...
                <input type="text" name="search" class="form-control rounded-3 font-14 shadow-none border -focus-dark" placeholder="{{ lang('Search…') }}" value="{{ request('search') ?? '' }}" required>
            </div>
        </form>
        <div class="sidebar-widget mb-30">
            ...
            <div class="category-list mt-30">
                ...
                @empty
                    <span class="text-muted">{{ lang('No categories found') }}</span>
                @endforelse
            </div>
        </div>
        <div class="sidebar-widget mb-30">
            ...
            @empty
                <span class="text-muted text-center">{{ lang('No articles found') }}</span>
            @endforelse
        </div>
        @if(!empty($blogTags))
        <div class="sidebar-widget mb-30 pb-20">
            <h4 class="widget-title">{{ lang('Tags') }}</h4>
