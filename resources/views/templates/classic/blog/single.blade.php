...
@section('og_image', asset('storage/blog/articles/'.$blog->image))
@section('content')
    <section class="blog-single pb-120">
                    ...
                    <div class="col-md-10 col-xl-8 mx-auto">
                        <div class="post-header">
                            ...
                            <ul class="post-meta mb-5 text-grey-2">
                                ...
                                <li class="post-author"><i class="fa-regular fa-user"></i><span>{{ lang('By') }} {{ $blog->user->name }}</span></li>
                                @if (@$settings->blog->commenting)
                                <li class="post-comments"><a href="#blog-comments"><i class="fa-regular fa-comment"></i>{{ $blogComments->count() }}<span> {{ lang('Comments') }}</span></a></li>
                                @endif
                            </ul>
                        </div>
                    </div>
        ...
        <div class="container">
            <div class="mx-auto wow fadeInUp mt-n100" data-wow-delay="300ms">
                <div class="row">
                    <div class="col-lg-12">
                        <div class="blog-image"><img class="w-100 h-auto rounded-3" src="{{ asset('storage/blog/articles/'.$blog->image) }}" alt="{{ $blog->title }}"></div>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="col-xl-8 offset-xl-2">
                    ...
                    <div class="d-block d-sm-flex justify-content-between pt-50 sm-pt-30 pb-50 sm-pb-30">
                        <div class="d-flex align-items-center sm-mb-10">
                            <span class="me-2">{{ lang('Share this post') }}</span>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="pag_prev">
                                    <a href="{{ route('blog.article', $previous_record->slug) }}">
                                        <h5><span class="fas fa-chevron-left pe-2"></span> {{ lang('Previous Post') }}</h5>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="pag_next">
                                    <a href="{{ route('blog.article', $next_record->slug) }}" class="text-end">
                                        <h5>{{ lang('Next Post') }}<span class="fas fa-chevron-right ps-2"></span></h5>
                                ...
                                <div class="col-lg-12">
                                    <div class="total-review d-flex align-items-center justify-content-between mb-20 mt-60">
                                        <h4 class="mb-15">
                                            ...
                                            {{ $blogComments->count() }} {{ lang('Comments') }}
                                        </h4>
                                    </div>
                                </div>
                                @forelse ($blogComments as $blogComment)
                                    <div class="col-md-12">
                                        <div class="position-relative d-flex align-items-center justify-content-start sm-mb-30">
                                            <img src="{{ asset('storage/avatars/users/'.$blogComment->user->avatar) }}"
                                                 alt="{{ $blogComment->user->name }}" class="mr-4 size-60 rounded-circle">
                                            <div class="ml-20">
                                                <h6 class="mt-0 mb-0">{{ $blogComment->user->name }}</h6>
                                                <div><span class="font-14">{{ date_formating($blogComment->created_at) }}</span></div>
                                            </div>
                                        </div>
                                        <p class="mt-20 mb-20">{!! nl2br($blogComment->comment) !!}</p>
                                    </div>
                                @empty
                                    <span class="text-muted mb-10">{{ lang('No comments found') }}</span>
                                @endforelse
                                <div class="separator-1px-op-l"></div>
                            </div>
                        </div>
                    ...
                    <div class="post-review">
                        <h6 class="font-18">{{ lang('Leave a comment') }}</h6>
                        @auth
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="form-group mb-24">
                                            <label class="mb-2">{{ lang('Comment') }}</label>
                                            <textarea name="comment" class="pt-15 form-control text-field" rows="6" placeholder="{{ lang('Your comment') }}" required></textarea>
                                        </div>
                                    </div>
                                    {!! display_captcha() !!}
                                    <div class="col-md-12">
                                        <button class="button -lg -primary push-right">{{ lang('Publish') }}<i class="ml-5 fal fa-arrow-right-long push-this"></i></button>
                                    </div>
                                </div>
                            </form>
                        ...
                        @else
                            <div class="card text-center">
                                <p>{{ lang('Login or create account to leave comments') }}</p>
                            </div>
                        @endauth
                    </div>
