    ...
    <div class="container">
                ...
                <div class="d-flex align-items-center font-16 gap-3">
                    @if ($footerMenuLinks->count() > 0)
                        @foreach ($footerMenuLinks as $footerMenuLink)
                            @php
                                if (!filter_var($footerMenuLink->link, FILTER_VALIDATE_URL)) {
                                    $footerMenuLink->link = url("/").$footerMenuLink->link;
                                }
                            @endphp
                            <div class="">
                                <a href="{{ $footerMenuLink->link }}" class="-underline fw-semibold">{{ $footerMenuLink->name }}</a>
                            </div>
                        @endforeach
                    @endif
                </div>
        ...
        <div class="row align-items-center g-4 g-md-0 pb-32">
            <div class="col-md-4 order-md-first order-last text-md-start text-center">
                © {{date('Y')}} {{ @$settings->site_title }} {{ lang('All rights reserved') }}.
            </div>
            <div class="col-md-4">
                <div class="d-flex font-16 gap-3 justify-content-center">
                    @if (@$settings->terms_of_service_link)
                        <div>
                            <a href="{{ @$settings->terms_of_service_link }}" class="text-dark-1 text-decoration -underline fw-semibold">{{ lang('Term & Condition') }}</a>
                        </div>
                    @endif

                    @if (@$settings->enable_contact_page)
                        <div>
                            <a href="{{ route('contact') }}" class="text-dark-1 text-decoration -underline fw-semibold">{{ lang('Contact') }}</a>
                        </div>
                    @endif
                </div>
            </div>
