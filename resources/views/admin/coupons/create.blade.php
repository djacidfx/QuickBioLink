<div class="slidePanel-content">
    ...
    <div class="slidePanel-inner">
        <form action="{{ route('admin.coupons.store') }}" method="post" enctype="multipart/form-data" id="sidePanel_form">
            @csrf
            <div class="mb-3">
                <label class="form-label" for="code">{{ lang('Coupon code') }} *</label>
                <div class="input-group mb-2">
                    <button class="btn btn-secondary copy-btn" type="button" data-clipboard-target="#couponCodeInput" data-tippy-placement="top" title="{{ lang('Copy') }}"><i class="icon-feather-copy"></i></button>
                    <input id="couponCodeInput" type="text" name="code" class="form-control"
                           placeholder="{{ lang('Coupon code') }}" maxlength="20" required>
                    <button id="generateCouponBtn" class="btn btn-primary" type="button"><i
                            class="fas fa-sync me-2"></i>{{ lang('Generate') }}</button>
                </div>
                <small class="form-text">{{ lang('Min 3 and max 20 characters allowed') }}</small>
            </div>
            <div class="mb-3">
                <label class="form-label" for="code">{{ lang('Discount percentage') }} *</label>
            ...
            <div class="mb-3">
                <label class="form-label" for="code">{{ lang('Limit for each user') }} *</label>
                <input type="number" name="limit" class="form-control" min="1"
                       placeholder="0" value="1" required>
            </div>
            <h5 class="mt-4">{{ lang('Usage details') }}</h5>
            <hr>
            <div class="mb-3">
                <label class="form-label" for="plan">{{ lang('Plan') }} *</label>
            ...
            <div class="mb-3">
                <label class="form-label" for="action_type">{{ lang('Action') }} *</label>
                <select name="action_type" id="action_type" class="form-control" required>
                    <option value="" disabled selected>{{ lang('Choose') }}</option>
                    <option value="0">{{ lang('All actions') }}</option>
                    <option value="1">{{ lang('Subscribing') }}</option>
                    <option value="2">{{ lang('Renewal') }}</option>
                    <option value="3">{{ lang('Upgrade') }}</option>
                </select>
            </div>
            <div class="mb-3">
                <label class="form-label" for="expiry_at">{{ lang('Expiry at') }} *</label>
