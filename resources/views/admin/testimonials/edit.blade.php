    ...
    <header class="slidePanel-header">
        <div class="slidePanel-overlay-panel">
            <div class="slidePanel-heading">
                <h2>{{ admin_lang('Edit Testimonial') }}</h2>
            </div>
        ...
        <form action="{{ route('admin.testimonials.update', $testimonial->id) }}" method="post" id="sidePanel_form" enctype="multipart/form-data">
            ...
            <div class="mb-3 form-group">
                ...
                <div class="translate-fields translate-fields-default">
                    <input name="designation" id="designation" type="text" class="form-control" required value="{{ $testimonial->designation }}" placeholder="{{ admin_lang('Designation') }}" autofocus>
                </div>
                @foreach ($adminLanguages as $language)
                    <div class="translate-fields translate-fields-{{ $language->code }}" style="display: none">
                        <input type="text" class="form-control" name="translations[{{ $language->code }}][designation]" placeholder="{{ admin_lang('Designation') }}" value="{{ !empty($testimonial->translations->{$language->code}->designation)
