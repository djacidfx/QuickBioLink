            ...
            <div class="mb-3 form-group">
                ...
                <div class="translate-fields translate-fields-default">
                    <input name="designation" id="designation" type="text" class="form-control" required
                           placeholder="{{ lang('Designation') }}" autofocus>
                </div>
                @foreach ($adminLanguages as $language)
                    <div class="translate-fields translate-fields-{{ $language->code }}" style="display: none">
                        <input type="text" class="form-control" name="translations[{{ $language->code }}][designation]" placeholder="{{ lang('Designation') }}">
                    </div>
                @endforeach
            </div>
            <div class="mb-3 form-group">
                ...
                <div class="translate-fields translate-fields-default">
                    <textarea name="content" class="form-control" required placeholder="{{ lang('Max 255 character') }}"></textarea>
                </div>
                @foreach ($adminLanguages as $language)
                    <div class="translate-fields translate-fields-{{ $language->code }}" style="display: none">
                        <textarea name="translations[{{ $language->code }}][content]" class="form-control" required placeholder="{{ lang('Max 255 character') }}"></textarea>
                    </div>
                @endforeach
            </div>
        </form>
