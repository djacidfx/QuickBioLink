...
@section('title', lang('Navbar Menu').' - '.$current_language)

@section('header_buttons')
    @include('admin.includes.language')
    <a href="#" data-url="{{route('admin.navbarMenu.create')}}" data-toggle="slidePanel"
       class="btn btn-info ms-2">{{ lang('Add New') }}</a>
    <form action="{{ route('admin.navbarMenu.nestable') }}" method="POST" class="d-inline-block">
        @csrf
        <input name="ids" id="ids" hidden>

        <button
            class="btn btn-primary ms-2 {{$links->count() == 0 ? 'disabled' : ''}}">{{ lang('Save') }}</button>
    </form>
@endsection
                            ...
                            <div class="dd-handle">
                                ...
                                <div class="dd-nodrag ms-auto d-flex">
                                    <a href="#" data-url="{{ route('admin.navbarMenu.edit', $link->id) }}"
                                       data-toggle="slidePanel" title="{{ lang('Edit') }}"
