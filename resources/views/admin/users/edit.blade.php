...
@section('title', admin_lang('Edit User'))
@section('content')
    <div class="row">
        ...
        <div class="col-lg-8 order-0 order-lg-1">
            <ul class="nav nav-pills flex-column flex-md-row mb-3">
                <li class="nav-item"><a class="nav-link active" href="javascript:void(0);"><i
                            class="icon-feather-user me-2"></i>{{ admin_lang('Account details') }}</a></li>
                ...
                <li class="nav-item"><a class="nav-link" href="{{ route('admin.users.logs', $user->id) }}"><i
                            class="icon-feather-list me-2"></i>{{ admin_lang('Login logs') }}</a></li>
            </ul>

            <form id="user-avatar-delete" class="d-none"
                  action="{{ route('admin.users.deleteAvatar', $user->id) }}" method="POST"
                  onsubmit='return confirm("{{admin_lang('Are you sure?').' \n' . admin_lang('It will refresh the page.')}}")'>
                @csrf
                @method('DELETE')
            </form>
            ...
            <form action="{{ route('admin.users.update', $user->id) }}" method="POST"
                ...
                <div class="quick-card card">
                    <div class="card-header">
                        <h5>{{ admin_lang('Account details') }}</h5>
                    </div>
                    <div class="card-body">
                        <div class="d-flex align-items-center gap-4 mb-4">
                            ...
                            <div class="button-wrapper">
                                <label for="upload" class="btn btn-primary me-2 mb-2" tabindex="0">
                                    ...
                                    <span class="d-none d-sm-block ms-2">{{ admin_lang('Upload new photo') }}</span>
                                ...
                                <button form="user-avatar-delete" class="btn btn-label-danger mb-2"><i
                                        ...
                                        class="d-none d-sm-block ms-2">{{ admin_lang('Remove') }}</span></button>

                                <p class="text-muted mb-0">{{ admin_lang('Allowed JPG, JPEG or PNG.') }}</p>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label class="form-label">{{ admin_lang('Firstname') }} *</label>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label class="form-label">{{ admin_lang('Lastname') }} *</label>
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="input-group mb-3">
                                        ...
                                        <button class="btn btn-primary quick-open-slide-panel" type="button"
                                                title="{{ admin_lang('Send Email') }}" data-tippy-placement="top"
                            ...
                            <div class="col-md-4">
                                <div class="mb-3">
                                    <label class="form-label">{{ admin_lang('User Type') }}</label>
                                    <select name="user_type" class="form-control" required>
                                        <option
                                            value="user" {{ ($user->user_type == 'user') ? "selected" : "" }}>{{ admin_lang('User') }}</option>
                                        <option
                                            value="admin" {{ ($user->user_type == 'admin') ? "selected" : "" }}>{{ admin_lang('Admin') }}</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="mb-3">
                                    ...
                                    <select name="status" class="form-control" required>
                                        <option
                                            value="1" {{ ($user->status == '1') ? "selected" : "" }}>{{ admin_lang('Active') }}</option>
                                        <option
                                            value="0" {{ ($user->status == '0') ? "selected" : "" }}>{{ admin_lang('Banned') }}</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="mb-3">
                                    ...
                                    <select name="email_status" class="form-control" required>
                                        <option value="0">{{ admin_lang('Unverified') }}</option>
                                        <option
                                            value="1" {{ (!is_null($user->email_verified_at)) ? "selected" : "" }}>{{ admin_lang('Verified') }}</option>
