<div class="quick-card card">
    ...
    <ul class="custom-list-group list-group list-group-flush border-top">
        ...
        <li class="list-group-item d-flex justify-content-between"><span>{{ admin_lang('Status') }} :</span>
            @if ($user->status)
        ...
        <li class="list-group-item d-flex justify-content-between"><span>{{ admin_lang('Email Verify') }} :</span>
            @if ($user->email_verified_at)
                <span class="badge bg-success">{{admin_lang('Verified')}}</span>
            @else
                <span class="badge bg-warning">{{admin_lang('Unverified')}}</span>
            @endif
        </li>
        ...
        <div class="card-body">
            ...
            <div class="d-grid w-100 mt-4 pt-2">
                <button class="btn btn-primary"
                        ...
                        data-toggle="slidePanel"><i class="icon-feather-gift me-2"></i>{{ admin_lang('Update Plan') }}
                </button>
            </div>
        </div>
    </div>
