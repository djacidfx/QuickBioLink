...
@section('title', lang('Blogs').' - '.$current_language)
@section('header_buttons')
    @include('admin.includes.language')
    <a href="{{ route('admin.articles.create') }}" class="btn btn-primary ms-2"><i class="icon-feather-plus me-2"></i> {{ lang('Add New') }}</a>
@endsection
@section('content')
            ...
            <div class="dataTables_wrapper">
                <table class="table table-striped" id="ajax_datatable" data-jsonfile="{{ route('admin.articles.index') }}?lang={{ $lang }}">
                    ...
                    <tr>
                        ...
                        <th class="tb-w-20x">{{ lang('Title') }}</th>
                        <th class="tb-w-7x">{{ lang('Category') }}</th>
                        <th class="tb-w-3x">{{ lang('Comments') }}</th>
                        <th class="tb-w-3x">{{ lang('Views') }}</th>
                        <th class="tb-w-7x">{{ lang('Published date') }}</th>
