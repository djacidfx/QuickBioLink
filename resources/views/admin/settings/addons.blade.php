                    ...
                    <div class="card accordion-item mb-3">
                        ...
                        <div id="recaptcha" class="accordion-collapse collapse" data-bs-parent="#accordions" style="">
                            <div class="accordion-body">
                                <div class="mb-3">
                                    <h5 class="mb-2">{{admin_lang('Instructions')}}</h5>
                                    <p class="mb-2">{{admin_lang('To find your Site Key and Secret Key, follow the below steps:')}}</p>
                                    <ol>
                                        <li><a href="https://www.google.com/recaptcha/admin/create" target="_blank">{{admin_lang('Go to the Google reCAPTCHA and register a new site.')}}</a></li>
                                        <li>{{admin_lang('Enter label and select "reCAPTCHA v2" -> "I\'m not a robot" Checkbox in reCAPTCHA type field.')}}</li>
                                        <li>{{admin_lang('Enter your domain url.')}}</li>
                                        <li>{{admin_lang('Accept Terms of Service and click on the Submit button.')}}</li>
                                        <li>{{admin_lang('Look for the Site Key and Secret Key. Use them in the form below on this page.')}}</li>
                                    </ol>
                                </div>
                                <div class="mb-3">
                                    {{quick_switch(admin_lang('Status'), 'google_recaptcha[status]', @$settings->google_recaptcha->status == '1')}}
                                </div>
                                <div class="row">
                                    <div class="col-sm-6">
                                        <div class="mb-3">
                                            <label class="form-label">{{ admin_lang('ReCAPTCHA Site Key') }} </label>
                                    ...
                                    <div class="col-sm-6">
                                        <div class="mb-3">
                                            <label class="form-label">{{ admin_lang('ReCAPTCHA Secret Key') }} </label>
                    ...
                    <div class="card accordion-item mb-3">
                        <h2 class="accordion-header" id="heading_analytics">
                            <button type="button" class="accordion-button fw-semibold collapsed" data-bs-toggle="collapse" data-bs-target="#analytics" aria-expanded="false" aria-controls="analytics">
                                {{ admin_lang('Google Analytics 4') }}
                            </button>
                        </h2>
                        <div id="analytics" class="accordion-collapse collapse" data-bs-parent="#accordions" style="">
                            <div class="accordion-body">
                                <div class="mb-3">
                                    {{quick_switch(admin_lang('Status'), 'google_analytics[status]', @$settings->google_analytics->status == '1')}}
                                </div>
                                <div class="mb-3">
                                    <label class="form-label">{{ admin_lang('Measurement Id') }} </label>
                    ...
                    <div class="card accordion-item mb-3">
                        <h2 class="accordion-header" id="heading_tawkto">
                            <button type="button" class="accordion-button fw-semibold collapsed" data-bs-toggle="collapse" data-bs-target="#tawkto" aria-expanded="false" aria-controls="tawkto">
                                {{ admin_lang('Tawk.to (Live Chat)') }}
                            </button>
                        </h2>
                        <div id="tawkto" class="accordion-collapse collapse" data-bs-parent="#accordions" style="">
                            <div class="accordion-body">
                                <div class="mb-3">
                                    {{quick_switch(admin_lang('Status'), 'tawk_to[status]', @$settings->tawk_to->status == '1')}}
                                </div>
                                <div class="mb-3">
                                    <label class="form-label">{{ admin_lang('Direct Chat Link') }} </label>
                                    <input type="text" name="tawk_to[chat_link]" class="form-control"
                                           value="{{ demo_mode() ? '' : @$settings->tawk_to->chat_link }}">
                                    <small class="form-text"><a href="https://help.tawk.to/article/direct-chat-link" target="_blank">{{admin_lang('You can find here.')}}</a></small>
                                </div>
                            </div>
                        </div>
                    </div>
