<div class="slidePanel-content">
    <header class="slidePanel-header">
        <div class="slidePanel-overlay-panel">
            <div class="slidePanel-heading">
                <h2>{{ admin_lang('Edit') .' '. $gateway->name }}</h2>
            </div>
    ...
    <div class="slidePanel-inner">
        <form action="{{ route('admin.gateways.update', $gateway->id) }}" method="post" id="sidePanel_form">
            ...
            <div class="d-flex align-items-start justify-content-between gap-4">
                <div>
                    <label for="upload" class="btn btn-primary mb-2" tabindex="0">
                        ...
                        <span class="d-none d-sm-block ms-2">{{ admin_lang('Change Logo') }}</span>
            ...
            <hr>
            <div class="mb-3">
                {{quick_switch(admin_lang('Active'), 'status', $gateway->status == '1')}}
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3">
                <label class="form-label">{{ admin_lang('Gateway fees') }} *</label>
            ...
            @if (!is_null($gateway->test_mode))
                <div class="mb-3">
                    {{quick_switch(admin_lang('Test Mode'), 'test_mode', $gateway->test_mode == '1')}}
                </div>
            @endif
            ...
            @switch($gateway->key)
                @case('paypal')
                <p>{{admin_lang('Get the API details from')}} <a href="https://developer.paypal.com/developer/applications/create" target="_blank">{{admin_lang('here')}} <i class="far fa-external-link"></i></a></p>
                @break

                @case('stripe')
                <p>{{admin_lang('Get the API details from')}} <a href="https://dashboard.stripe.com/apikeys" target="_blank">{{admin_lang('here')}} <i class="far fa-external-link"></i></a></p>
                @break

                @case('mollie')
                <p>{{admin_lang('Get the API details from')}} <a href="https://www.mollie.com/dashboard" target="_blank">{{admin_lang('here')}} <i class="far fa-external-link"></i></a></p>
                @break

                @case('razorpay')
                <p>{{admin_lang('Get the API details from')}} <a href="https://dashboard.razorpay.com/app/keys" target="_blank">{{admin_lang('here')}} <i class="far fa-external-link"></i></a></p>
                @break
            @endswitch
        </form>
    </div>
